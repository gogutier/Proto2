...
def invsimple2(request):
    ...
    #los pallets que están en esa ubicación según CTI, pero exluyendo los que ya están en palletstomainv
    palletscti= Pallet.objects.filter(ubic="antes decía tomainv.aux1.upper() ")#.exclude(tarja__in=[o.tarja for o in palletstomainv]).order_by('pk')
    ...
    palletsencontrados = PalletCic.objects.filter(ubic=" antes decía tomainv.aux1.upper()", tomainvcic=tomainv)#.exclude(tarja__in=[o.tarja for o in palletsnoencontrados]).order_by('pk')
